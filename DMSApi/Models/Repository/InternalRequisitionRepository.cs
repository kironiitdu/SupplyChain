...
{
    public class InternalRequisitionRepository : IInternalRequisitionRepository
    {
        private DMSEntities _entities;
        private PartyJournalRepository partyJournalRepository;

        public InternalRequisitionRepository()
        {
        ...
        public object GetAllInternalRequisitionRfd()
        {
            try
            {
                var data = (from dm in _entities.delivery_master
                            ...
                            join r in _entities.internal_requisition_master on dm.requisition_master_id equals
                                r.internal_requisition_master_id into tempr
                            from r in tempr.DefaultIfEmpty()
                            where dm.status == "CASH-RFD" 
                            select new
                            {
        ...
        public object GetAllInternalDeliveryList()
        {
            try
            {
                var data = (from dm in _entities.delivery_master
                            ...
                            from r in tempr.DefaultIfEmpty()
                            where dm.status == "CASH-Delivered" && p.party_name == "Internal (Cash)"
                            select new
                            {
                                delivery_master_id = dm.delivery_master_id,
                                delivery_no = dm.delivery_no,
                                ...
                                courier_slip_no = dm.courier_slip_no,
                                requisition_code = r.internal_requisition_no,
                                internal_requisition_master_id = dm.requisition_master_id,
                                created_date = r.created_date,
                                receive_date = dm.receive_date //== null ? "Pending" : dm.receive_date.ToString()
